...
function getCaptcha(urlCaptcha) {
    $self.getCaptcha(urlCaptcha).then((res) => {
        var captchaBase64 = new Buffer(res.data, 'binary').toString('base64')

        $self.parseBase64ImageToString(captchaBase64).then((res) => {
...
function performAutoRequest(stringCaptcha) {
    ...
    $self.postAutoRequest(ID, stringCaptcha).then((res) => {
